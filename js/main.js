...
$(document).ready(function() {
	...
	$("body").on("change input", ".count-input", function () {
	...
	$(".about-text-slider").on("init", function () {

		$(".about-arrow-prev").click(function () {

			$(".about-text-slider").slick("slickPrev");

		});

		$(".about-arrow-next").click(function () {

			$(".about-text-slider").slick("slickNext");

		});

	});

	$(".about-text-slider").slick({
		slidesToShow: 1,
		slidesToScroll: 1,
		speed: 500,
		rows: 0,
	...
	$(".modal").click(function (e) {
		...
		if ($(this).find(".who-item").length) {
			...
			if (!$(e.target).hasClass("modal-body") && !$(e.target).parents().hasClass("modal-body")) {
	...
	$("body").on("click", ".cart-item-remove", function () {
		...
		thisBtn.closest(".cart-item").fadeOut(150, function () {

			thisBtn.closest(".cart-item").remove();
	...
	$("#orderForm").submit(function() {
		if ($(this).valid()) {
			...
			$.ajax({
				...
				data: {
					subject: "Одноразовые маски и средства защиты - заявка",
					formname: "под верхним блоком",
					name: $("#order_name").val(),
	...
	$("#orderForm2").submit(function() {
		if ($(this).valid()) {
			...
			$.ajax({
				...
				url: baseUrl + "order.php",
				data: {
					subject: "Одноразовые маски и средства защиты - заявка (скидка)",
					formname: "скидка 15%",
	...
	$(".serv-tmb [data-toggle='modal']").click(function () {
		
		$("#order_modal_subject").val("Одноразовые маски и средства защиты - заказ услуги");
		$("#order_modal_service").val($(this).closest(".serv-tmb").find(".serv-tmb-descr h3").html());

	});

	$(".tech-tmb [data-toggle='modal']").click(function () {

		$("#order_modal_subject").val("Одноразовые маски и средства защиты - заказ услуги (технологии)");
		$("#order_modal_service").val($(this).closest(".tech-tmb").find("h3").html());

	});

	$('#orderModal').on('hidden.bs.modal', function () {
		$("#order_modal_subject").val("");
		$("#order_modal_service").val("");
	})
	...
	$("#orderFormModal").submit(function() {
		if ($(this).valid()) {
			...
			$.ajax({
				...
				url: baseUrl + "order.php",
				data: {
					subject: $("#order_modal_subject").val(),
					service: $("#order_modal_service").val(),
					name: $("#order_modal_name").val(),
					phone: $("#order_modal_phone").val(),
					where: $("#order_modal_where option[value='" + $("#order_modal_where").val() + "']").text()
				}
			}).done(function() {
...
function contactsMap() {
	if ($("#contactsMap").length) {
		...
		if ($("#mobile-indicator").css("display") == "block") {
			...
			map.setCenter({lat:pinLat, lng:pinLng});

		} else {

			map.setCenter({lat:pinLat, lng:pinLng - 0.005});
...
function initCart() {
	...
	$(".cart-item").each(function () {
		...
		var thisPrice = $(this).data("price") * 1;

		var thisTotalPrice = thisPrice * $(this).find(".count-input").val();

		$(this).find(".cart-item-total .price").html(numFormat.to(thisTotalPrice));
